...
from .forms import ProyectoForm,SocialNetworkForm
...
@admin.route('/redsocial',methods=['GET','POST'])
def social_network():
    if('token' not in session):
        return redirect(url_for('admin.login'))
    
    list_social_networks = fb.get_collection('redes_sociales')
    social_network_form = SocialNetworkForm()
    
    if social_network_form.validate_on_submit():
        #registrar nuevo proyecto
        data_new_social_network = {
            'icono': social_network_form.icono.data,
            'url':social_network_form.url.data,
        }
        new_social_newtwork_doc = fb.insert_document('redes_sociales',data_new_social_network)
        
        return redirect(url_for('admin.social_network'))
    
    context = {
        'social_networks':list_social_networks,
        'social_network_form':social_network_form
    }
    return render_template('admin/redsocial.html',**context)

@admin.route('/redsocial/<id>',methods=['GET','POST'])
def social_network_by_id(id=''):
    if('token' not in session):
        return redirect(url_for('admin.login'))
    
    list_social_networks = fb.get_collection('redes_sociales')
    social_network_data = fb.get_document('redes_sociales',id)
    
    social_network_form = SocialNetworkForm(data=social_network_data)
    ...
    if social_network_form.validate_on_submit():
        ...
        social_network_updated = fb.update_document('redes_sociales',id,data_new_social_network)
...
@admin.route('/delredsocial/<id>')
def del_social_network(id=''):
    if('token' not in session):
        return redirect(url_for('admin.login'))
    
    del_social_network_result = fb.delete_document('redes_sociales',id)
    if(del_social_network_result == True):
        return redirect(url_for('admin.social_network'))
